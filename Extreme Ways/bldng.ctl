...
Dim m_BaseX(100) As Integer
Dim m_BaseY(100) As Integer
...
Dim m_Size(100) As Double
Dim m_Mass(100) As Integer
...
m_BaseX(n) = Val(GetFromIni(m_bldng(n), "BaseX", pth))
m_BaseY(n) = Val(GetFromIni(m_bldng(n), "BaseY", pth))
...
m_Size(n) = Val(GetFromIni(m_bldng(n), "Size", pth))
m_Mass(n) = Val(GetFromIni(m_bldng(n), "Mass", pth))
...
Next
End Function
Function cost(bldng As String) As Long
Dim n As Integer
For n = 1 To many
If UCase(m_bldng(n)) = UCase(bldng) Then
cost = m_cost(n)
...
Function BaseX(bldng As String) As Integer
Dim n As Integer
For n = 1 To many
If UCase(m_bldng(n)) = UCase(bldng) Then
BaseX = m_BaseX(n)
...
Next
End Function
Function BaseY(bldng As String) As Integer
Dim n As Integer
For n = 1 To many
If UCase(m_bldng(n)) = UCase(bldng) Then
BaseY = m_BaseY(n)
...
Function Size(bldng As String) As Double
Dim n As Integer
For n = 1 To many
If UCase(m_bldng(n)) = UCase(bldng) Then
Size = m_Size(n)
...
Function Mass(bldng As String) As Integer
Dim n As Integer
For n = 1 To many
If UCase(m_bldng(n)) = UCase(bldng) Then
Mass = m_Mass(n)
